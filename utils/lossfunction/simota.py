...
class SimOTA:
    ...
    def __call__(self, outputs, targets):
        # -------------------------------------------------------------
        # PART 1: PRE-PROCESSING (This part is correct)
        # -------------------------------------------------------------
        ...
        # ADD THIS LINE:
        scores = scores.clamp(min=self.eps, max=1-self.eps)
        ...
        num_pos = fg_mask.sum().item() # Convert to a standard Python integer
        
        # -------------------------------------------------------------
        # PART 2: CONSOLIDATED LOSS CALCULATION (This is the clean, correct part)
        # -------------------------------------------------------------
        ...
        if num_pos > 0:
            ...
            neg_scores = torch.cat((pred_scores[fg_mask][~mask], pred_scores[~fg_mask].view(-1)), dim=0).sigmoid().clamp(min=self.eps, max=1-self.eps)
            gamma = self.gamma
        ...
        # -------------------------------------------------------------
        # PART 3: FINAL SCALING AND RETURN
        # -------------------------------------------------------------
        # Scale losses and return the total
        ...
        if num_pos == 0:
            # Return a loss tied to the computational graph to avoid RuntimeError
    ...
    def assign(self, pred_scores, pred_bboxes, true_labels, true_bboxes, true_mask, anchors, stride_tensor):
        ...
        if self.num_max_boxes == 0:
            ...
            # target_bboxes = torch.full_like(pred_scores[..., 0], self.nc).to(device)
            # target_scores = torch.zeros_like(pred_bboxes).to(device)
            # fg_mask = torch.zeros_like(pred_scores, dtype=torch.int).to(device)

            target_bboxes = torch.empty((self.bs, num_preds, 4), device=device)
            target_scores = torch.zeros((self.bs, num_preds, self.nc), device=device)
            fg_mask = torch.zeros((self.bs, num_preds), dtype=torch.bool, device=device)
        ...
        # [B, nbox, 1029]
        overlaps = overlaps.squeeze(3).clamp(min=0)
        # Now use a clamp with a small value for the log:
        iou_loss = -torch.log(overlaps.clamp(min=self.eps))
